...
import {
  Users, MessageSquare, ClipboardList, UsersRound, Megaphone,
  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="px-6 py-4 bg-white border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500">Visão geral do CRM em números</p>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div className="max-w-6xl mx-auto flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <BigCard icon={Users}        label="Contatos"          value={data.contatos.total} hint={`+${data.contatos.novos30} em 30 dias`} color="#6366f1" />
          ...
          <section className="bg-white border border-gray-200 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
              <TrendingUp size={14} className="text-emerald-600" /> Crescimento de contatos
            </h2>
          ...
          {/* Por papel */}
          <section className="bg-white border border-gray-200 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-700 mb-4">Contatos por papel</h2>
            <div className="space-y-2">
              {data.contatos.porPapel.length === 0 && <p className="text-xs text-gray-400">Sem dados</p>}
              {data.contatos.porPapel.map((p: any) => (
                <Bar key={p.roleId} label={p.label} value={p.count} max={data.contatos.total} color={p.color} bg={p.bgColor} />
              ))}
            </div>
          </section>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Conversas por kanban */}
            <section className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <section className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <p className="text-xs text-gray-400 mb-3">{data.campanhas.total} campanha(s) cadastrada(s) no total (incluindo disparos de reunião)</p>
...
function Bar({ label, value, max, color, bg }: any) {
