...
export async function GET() {
  const [roles, bairrosRaw, cidadesRaw, labelsRaw] = await Promise.all([
    ...
    prisma.label.findMany({ select: { name: true, color: true, bgColor: true } }),
  ]);

  // labels usadas em contatos (com count)
  // Como labels é Contact.labels[] (array de string), agrupar via SQL puro
  ...
  labelsRaw.forEach(l => labelsMap.set(l.name, { color: l.color, bgColor: l.bgColor }));
